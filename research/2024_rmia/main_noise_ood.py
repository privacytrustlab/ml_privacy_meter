        ...
        for model_index in list_target_models:
            ...
            # if not os.path.isfile(results_path): # load saved results for model_index
                ...
                if is_extra:
                    ...
                    members_indices = torch.nonzero(membership[np.arange(dataset_size)]).squeeze()
                    non_member_indices = dataset_size + torch.nonzero(membership[np.arange(dataset_size)]).squeeze() # noisy members as OOD non members
                    target_and_test_idx = torch.cat((members_indices, non_member_indices), dim=0)
                    population_indices = np.setdiff1d(np.arange(dataset_size), target_and_test_idx) # normal non-members indices
