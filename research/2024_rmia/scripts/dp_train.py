...
class TrainLoop(objax.Module):
    ...
    def train(self, num_train_epochs: int, train_size: int, train: DataSet, test: DataSet, logdir: str, save_steps=100, patience=None):
        ...
        """
        sampling_rate = FLAGS.batch / float(train_size)  # for privacy accounting.
        all_steps = 0 # for privacy accounting.
        ...
        with SummaryWriter(os.path.join(logdir, 'tb')) as tensorboard:
            for epoch in range(start_epoch, num_train_epochs):
                ...
                for step in loop:
                    ...
                    current = next(train_iter)
                    self.train_step(summary, current, progress)
                    all_steps += 1

                # Eval
                accuracy, total = 0, 0
                train_accuracy, train_total = 0, 0
                if epoch%FLAGS.eval_steps == 0 and test is not None:
                    ...
                    ############################ Added
                    train_iters = iter(train)
                    for _ in range(10):
                        current = next(train_iters)
                        train_total += current['image'].shape[0]
                        train_preds = np.argmax(self.predict(current['image'].numpy()), axis=1)
                        train_accuracy += (train_preds == np.argmax(current['label'].numpy(), axis=1)).sum()
                    ############################ Added
                    ...
                    eps = objax.privacy.dpsgd.analyze_dp(q=sampling_rate,
                                             ...
                                             steps=all_steps, delta=FLAGS.delta)
                    ############################ Added for DP-SGD

                    accuracy /= total
                    train_accuracy /= train_total
                    summary.scalar('eval/accuracy', 100 * accuracy)
                    summary.scalar('eval/train_accuracy', 100 * train_accuracy)
                    summary.scalar('privacy/epsilon', eps) ############################ Added for DP-SGD
                    tensorboard.write(summary, step=(epoch + 1) * train_size)
                    print('Epoch %04d  Loss %.2f Test Accuracy %.2f Train Accuracy %.2f Epsilon %.2f Delta %.2f' % (epoch + 1, summary['losses/xe'](),
                                                                    summary['eval/accuracy'](), summary['eval/train_accuracy'](), eps, FLAGS.delta))
...
class MemModule(TrainLoop):
    def __init__(self, model: Callable, nclass: int, mnist=False, **kwargs):
        ...
        self.model = model(1 if mnist else 600 if FLAGS.dataset == 'purchase100' else 3, nclass) # init with number of channels # Modified
        ...
        gv = objax.privacy.dpsgd.PrivateGradValues(loss, self.model.vars(),
        ...
        def train_op(progress, x, y):
            g, v = gv(x, y)
            lr = self.params.lr
            # lr = self.params.lr * jn.cos(progress * (7 * jn.pi) / (2 * 8)) # no cosine lr
...
def network(arch: str):
    ...
    ############################ Added
        ...
        return functools.partial(MLPerceptron, layer_units=[512, 256, 128, 64])
    ############################ Added
...
def get_data(seed):
    ...
    if os.path.exists(os.path.join(FLAGS.logdir, "x_train.npy")) or FLAGS.dataset == "purchase100" or FLAGS.dataset == "cinic10": # Modified
        try:
        ...
        ############################ Added Part for DP only : we exacerbate the memorization of some points by flipping their labels
        def flip_labels(labels, indices_to_flip):
            np.random.seed(0)
            returned = labels.copy()
            for index in indices_to_flip:
                original_label = returned[index]
                possible_labels = np.array([label for label in range(10) if label != original_label])
                replacement_label = np.random.choice(possible_labels)
                returned[index] = replacement_label
            return returned

        np.random.seed(0)
        idx_to_flip = np.random.choice(np.arange(50000), size=1000, replace=False)
        labels = flip_labels(labels, idx_to_flip)
            
        inputs = (inputs/127.5)-1
        np.save(os.path.join(FLAGS.logdir, "x_train.npy"),inputs)
        np.save(os.path.join(FLAGS.logdir, "y_train.npy"),labels)

        np.save(os.path.join("exp/", "idx_to_flip.npy"),idx_to_flip) # will be used in the "susbset" parameter, when set to dp outliers
        ############################ Added 
    ...
    if FLAGS.dataset == "cinic10":
        ...
        test = DataSet.from_arrays(x_test, y_test, augment_fn=lambda x: augment(x, 0, mirror=False))
    ...
    elif FLAGS.dataset == "purchase100":
        try:
            x_test = np.load(os.path.join(FLAGS.logdir, "x_test.npy"))
            y_test = np.load(os.path.join(FLAGS.logdir, "y_test.npy"))
        except:
...
if __name__ == '__main__':
    ...
    flags.DEFINE_float("noise_multiplier", 1.1, "Noise multiplier.")
    flags.DEFINE_integer("microbatch", 1, "Microbatch size.")
    flags.DEFINE_float("delta", 1e-5, "Target delta value for DP.")
    ### For DP-SGD ###
