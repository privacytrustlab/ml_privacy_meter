...
def get_rocs_from(model_idx, attack_list_and_paths, log_dir, save_path, colors=None, log=False):
    attacks_stats = {}
    aucs_list = {}
    nb_decimals = 3
    ...
    for i, (attack, pa) in enumerate(attack_list_and_paths):
        attacks_stats[attack] = get_fpr_tpr(log_dir=log_dir, report_folder=pa, model_idx=model_idx)
        aucs_list[attack] = np.round(metric_results(attacks_stats[attack][0], attacks_stats[attack][1])[0], nb_decimals)
        
        if colors is not None:
            plt.plot(attacks_stats[attack][0], attacks_stats[attack][1], color=colors[i], label=f"{attack} - AUC: {aucs_list[attack]}")
        else:
...
if __name__ == "__main__":
    ...
    elif args.figure == 301:
        log_dir = f"demo_cifar10"
        model_idx = 0 # 12# "all" # "all" # 0 # "all"
        attacks_stats = {}
        attack_list_and_paths = [
                        ("relative", "report_relative_offline_full_ref_model_ood"),
                        ("lira", "report_lira_offline_full_ref_model_ood"),
                        ("reference", "report_reference_full_ref_model_ood"),
                        ("population", "report_population_ood"),
                    ]
        folder = "../../paper/data/rocs"
        path = "rocs_ood"
        get_csv_rocs_from(model_idx, attack_list_and_paths, log_dir, path, folder)
        get_rocs_from(model_idx, attack_list_and_paths, log_dir, f'{log_dir}/rocs_ood_log.png', log=True)
    
    elif args.figure == 302:
        log_dir = f"demo_cifar10"
        model_idx = 0 # 12# "all" # "all" # 0 # "all"
        attacks_stats = {}
        attack_list_and_paths = [
                        ("relative", "report_relative_offline_full_ref_model_noise"),
                        ("lira", "report_lira_offline_full_ref_model_noise"),
                        ("reference", "report_reference_full_ref_model_noise"),
                        ("population", "report_population_noise"),
                    ]
        folder = "../../paper/data/rocs"
        path = "rocs_noise"
        get_csv_rocs_from(model_idx, attack_list_and_paths, log_dir, path, folder)
        get_rocs_from(model_idx, attack_list_and_paths, log_dir, f'{log_dir}/rocs_noise_log.png', log=True)
    
    elif args.figure == 401:
        log_dir = f"demo_cifar10"
        model_idx = 0 # 12# "all" # "all" # 0 # "all"
        attacks_stats = {}
        prefix="cifar10_noisy_members_scale_0.1"
        attack_list_and_paths = [
                        ("relative", f"{prefix}_rmia"),
                        ("lira", f"{prefix}_lira_offline"),
                        ("reference", f"{prefix}_reference"),
                        ("population", f"{prefix}_population"),
                    ]
        folder = "../../paper/data/rocs"
        path = prefix
        get_csv_rocs_from(model_idx, attack_list_and_paths, log_dir, path, folder)
        get_rocs_from(model_idx, attack_list_and_paths, log_dir, f'{log_dir}/rocs_{prefix}_log.png', log=True)
        get_rocs_from(model_idx, attack_list_and_paths, log_dir, f'{log_dir}/rocs_{prefix}.png', log=False)
    ...
    elif args.figure == 501: # gradients
        log_dir = f"demo_mia_gradients" # 64 reference models
        model_idx = 0 # 12# "all" # "all" # 0 # "all"
        attacks_stats = {}
        prefix = "report"
        attack_list_and_paths = [
                        ("relative", f"report_rmia"),
                        ("lira", f"report_lira"),   
                        ("gradient", f"report_gradients"),
                        ("population", f"report_population"),
                        ("gradient norm.", f"report_gradients_normalized")
        ]
        
        folder = "../../paper/data/rocs"
        path = prefix
        colors = np.array(["blue","red","green","orange","purple"])
        get_rocs_from(model_idx, attack_list_and_paths, log_dir, f'{log_dir}/rocs_gradient_log.png', colors=colors, log=True)
        get_rocs_from(model_idx, attack_list_and_paths, log_dir, f'{log_dir}/rocs_gradient.png', colors=colors, log=False)
