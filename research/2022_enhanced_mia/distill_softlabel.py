...
# Modifications by yuan74
# Add reading operation from prediction file
# perform distillation on soft labels
...
from objax.zoo import convnet, wide_resnet, dnnet, vgg
import customvgg
...
class MemModule(TrainLoop):
    def __init__(self, model: Callable, nclass: int, mnist=False, **kwargs):
        ...
        self.opt = objax.optimizer.Adam(self.model.vars(), eps=1e-7)
        ...
        self.predict = objax.Jit(objax.nn.Sequential([objax.ForceArgs(self.model, training=False)]))
...
def network(arch: str):
    if arch == 'cnn32-2-max':
        return functools.partial(convnet.ConvNet, scales=2, filters=32, filters_max=64,
        ...
        return functools.partial(vgg.VGG19, pretrained=False)
    elif arch == 'vgg3':
        return functools.partial(customvgg.CustomVgg)
...
def get_data(seed):
    ...
    if os.path.exists(os.path.join(FLAGS.logdir, "y_softlabel.npy")):
        inputs = np.load(os.path.join(FLAGS.logdir, "x_train.npy"))
        labels = np.load(os.path.join(FLAGS.logdir, "y_softlabel.npy"))
    else:
        ...
        labels = np.load(os.path.join(FLAGS.logdir, "experiment-z_target_model", "predictions", "0000000100.npy"))[:,0,0,:]
        # attack dummy soft labels for records not predicted in inference step
        dummylabels = np.zeros((len(inputs) - len(labels), len(labels[0])))
        labels = np.concatenate((labels, dummylabels), axis = 0)
            
        inputs = (inputs/127.5)-1
        np.save(os.path.join(FLAGS.logdir, "x_train.npy"),inputs)
        np.save(os.path.join(FLAGS.logdir, "y_softlabel.npy"),labels)
...
def main(argv):
    ...
    tm = MemModule(model=network(arch=FLAGS.arch), nclass=nclass,
