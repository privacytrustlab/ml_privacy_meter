...
def sample_data_imputation(range_center, sample_size, num_missing_feats, col_range, is_categorical=False):
    """
    ...
    def impute_missing(candidate):
        ...
        if is_categorical:
            ...
            imputed = np.array([np.random.randint(int(low), int(high) + 1) for low, high in zip(lows, highs)])
        else:
