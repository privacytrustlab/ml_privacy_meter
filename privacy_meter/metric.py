...
class PopulationMetric(Metric):
    ...
    def __init__(
            ...
            target_model_to_train_split_mapping: List[Tuple[int,
                                                            str, str, str]] = None,
            target_model_to_test_split_mapping: List[Tuple[int,
                                                           str, str, str]] = None,
            reference_model_to_train_split_mapping: List[Tuple[int,
                                                               str, str, str]] = None,
    ...
    def run_metric(self, fpr_tolerance_rate_list=None) -> List[MetricResult]:
        ...
        thresholds = self.hypothesis_test_func(
            self.reference_signals, self.quantiles).reshape(-1, 1)

        num_threshold = len(self.quantiles)
        member_signals = self.member_signals.reshape(
            -1, 1).repeat(num_threshold, 1).T
        non_member_signals = self.non_member_signals.reshape(
            -1, 1).repeat(num_threshold, 1).T
...
class ShadowMetric(Metric):
    ...
    def __init__(
            ...
            reference_model_to_train_split_mapping: List[Tuple[int,
                                                               str, str, str]] = None,
            reference_model_to_test_split_mapping: List[Tuple[int,
                                                              str, str, str]] = None,
    ...
    def prepare_metric(self):
        ...
        """
        # Load signals if they have been computed already; otherwise, compute and save them
        self.member_signals = flatten_array(self._load_or_compute_signal(
            signal_source=SignalSourceEnum.TARGET_MEMBER))
        self.non_member_signals = flatten_array(
            self._load_or_compute_signal(SignalSourceEnum.TARGET_NON_MEMBER))
    ...
    def run_metric(self, fpr_tolerance_rate_list=None) -> MetricResult:
        ...
        x = np.concatenate([self.reference_member_signals,
                           self.reference_non_member_signals]).reshape(-1, 1)
        y = np.array([1] * len(self.reference_member_signals) +
                     [0] * len(self.reference_non_member_signals))
        clf.fit(x, y)

        signal_space = np.linspace(np.array(x).ravel().min(
        ), np.array(x).ravel().max(), 200).reshape((-1, 1))
        i = np.max([i if v == 1 else -1 for i,
                   v in enumerate(clf.predict(signal_space))])
...
class ReferenceMetric(Metric):
    ...
    def run_metric(self, fpr_tolerance_rate_list=None) -> List[MetricResult]:
        ...
        non_member_preds = np.less(
            non_member_signals, reference_non_member_threshold)
...
class GroupPopulationMetric(Metric):
    ...
    def __init__(
            ...
            target_model_to_test_split_mapping_group: List[Tuple[int,
                                                                 str, str]] = None,
    ...
    def run_metric(self, fpr_tolerance_rate_list=None) -> List[MetricResult]:
        ...
        for g in np.unique(self.reference_groups):
            ...
            thresholds = self.hypothesis_test_func(
                self.reference_signals[self.reference_groups == g], self.quantiles).reshape(-1, 1)

            member_signals = self.member_signals[self.member_groups == g].reshape(
                -1, 1).repeat(num_threshold, 1).T

            non_member_signals = self.non_member_signals[self.non_member_groups == g].reshape(
                -1, 1).repeat(num_threshold, 1).T
            member_pred = np.less(member_signals, thresholds)
            non_member_pred = np.less(non_member_signals, thresholds)

            member_signal_list.append(
        ...
        member_signals = np.concatenate(
            member_signal_list, axis=0)  # reoder based on the groups
