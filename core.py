...
def prepare_datasets(dataset_size, num_target_model, configs, model_metadata_list, matched_idx=None):
    ...
    if matched_idx is not None:
        for metadata_idx in matched_idx:
            ...
            if 'test_split' in metadata:
                index_list.append(
                    {'train': metadata['train_split'], 'test': metadata['test_split'], 'audit': metadata['audit_split']})
            else:
                rest_index = np.array(
                    [i for i in all_index if i not in metadata['train_split']])
                test_index = np.random.choice(rest_index, int(
                    configs['f_test']*dataset_size), replace=False)
                used_index = np.concatenate(
                    [metadata['train_split'], test_index])
                audit_index = get_split(all_index, used_index, size=int(
                    configs['f_audit']*dataset_size), split_method=configs['split_method'])
                index_list.append(
                    {'train': metadata['train_split'], 'test': test_index, 'audit': audit_index})
    ...
    for split in range(num_addition_dataset):
        selected_index = np.random.choice(all_index, int(
            (configs['f_train']+configs['f_test'])*dataset_size), replace=False)
        train_index, test_index = train_test_split(
            selected_index, test_size=configs['f_test']/(configs['f_train']+configs['f_test']))
        audit_index = get_split(all_index, selected_index, size=int(
            configs['f_audit']*dataset_size), split_method=configs['split_method'])
        index_list.append(
...
def prepare_datasets_for_sample_privacy_risk(N, num_total, num_models, data_idx, configs, data_type, model_metadata_list, matched_in_idx=None):
    ...
    if configs['split_method'] == 'uniform':
        ...
        for _ in range(num_models):
            if data_type == 'include':
                train_index = np.random.choice(all_index_exclude_z, int(
                    (configs['f_train'])*N)-1, replace=False)
                index_list.append({'train': np.append(
                    train_index, data_idx), 'test': all_index, 'audit': all_index})
            elif data_type == 'exclude':
                train_index = np.random.choice(all_index_exclude_z, int(
                    (configs['f_train'])*N), replace=False)
                index_list.append(
    ...
    # We generate a list of dataset which is the same as the training dataset of the models indicated by the matched_in_idx but excluding the target point.
        ...
        assert len(
            matched_in_idx) >= num_models, "Input enough in-world (with the target point z) to generate the out world"
        ...
        for metadata_idx in matched_in_idx:
            metadata = model_metadata_list['model_metadata'][metadata_idx]
            train_index = np.delete(metadata['train_split'], np.where(
                metadata['train_split'] == data_idx)[0])
    ...
    dataset_splits = {'split': index_list,
                      'split_method': configs['split_method'], 'associated_models': associated_models}
    return dataset_splits
...
def prepare_models(log_dir, dataset, data_split, configs, model_metadata_list, matched_idx=None):
    ...
    for split in range(len(model_list), len(data_split['split'])):
        ...
        test_loader = torch.utils.data.DataLoader(
            test_data, batch_size=configs['test_batch_size'], shuffle=False, num_workers=2)

        print(50*"-")
        print(
            f'Training the {split}-th model: the training dataset of size {len(train_data)} and test dataset of size {len(test_data)}')
        ...
        logging.info(
            f'Prepare {split}-th target model costs {time.time()-baseline_time} seconds: Train accuracy {train_acc}, Train Loss {train_loss}; Test accuracy {test_acc}, Test Loss {test_loss}')
        print(50*"-")
...
def get_info_source_population_attack(dataset, data_split, model, configs):
    ...
    target_model = PytorchModelTensor(model_obj=model, loss_fn=nn.CrossEntropyLoss(
    ), device=configs['device'], batch_size=configs['audit_batch_size'])
...
def get_info_source_reference_attack(log_dir, dataset, data_split, model, configs, model_metadata_list, matched_reference_idx=None):
    ...
    if matched_reference_idx is not None:
        for metadata_idx in matched_reference_idx:
            ...
            with open(f"{metadata['model_path']}", 'rb') as f:
                model_weight = pickle.load(f)
            model.load_state_dict(model_weight)
            reference_models.append(PytorchModelTensor(model_obj=copy.deepcopy(
                model), loss_fn=nn.CrossEntropyLoss(), device=configs['device'], batch_size=configs['audit_batch_size']))
    ...
    for reference_idx in range(num_reference_models):
        reference_data_idx = get_split(data_split['audit'], None, size=int(
            configs['f_reference_dataset']*len(train_data)), split_method=configs['split_method'])
        ...
        meta_data['model_path'] = f'{log_dir}/model_{model_idx}.pkl'
        model_metadata_list['model_metadata'][model_idx] = meta_data
        reference_models.append(PytorchModelTensor(model_obj=reference_model, loss_fn=nn.CrossEntropyLoss(
        ), device=configs['device'], batch_size=configs['audit_batch_size']))
    ...
    for split in range(len(model_list)):
        ...
        if configs['algorithm'] == 'population':
            ...
            if matched_reference_idx != None and len(matched_reference_idx) > split:
                target_dataset, audit_dataset, target_model, audit_models, model_metadata_list = get_info_source_reference_attack(
                    log_dir, dataset, data_split['split'][split], model_list[split], configs, model_metadata_list, matched_reference_idx[split])
            else:
                target_dataset, audit_dataset, target_model, audit_models, model_metadata_list = get_info_source_reference_attack(
                    log_dir, dataset, data_split['split'][split], model_list[split], configs, model_metadata_list)
