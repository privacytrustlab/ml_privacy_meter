...
test_data = [
    (
        ...
        {
            "model_metadata": {
                1: {"condition1": "value1", "condition2": "value2"},
                2: {"condition3": "value3", "condition4": "value4"},
                3: {"condition1": "value1", "condition3": "value3"},
            }
        },
        2,
        [1],
...
@pytest.mark.parametrize(
    "conditions, model_metadata_list, \
                          num_models, target_idx, expected",
    test_data,
)
def test_load_models_by_conditions(
    conditions, model_metadata_list, num_models, target_idx, expected
):
    # Act
...
model_metadata_list = {
    "model_metadata": {
        1: {"model_idx": 6, "train_split": [1, 2, 3]},
        2: {"model_idx": 7, "train_split": [1, 4, 7]},
        3: {"model_idx": 8, "train_split": [1, 5, 8]},
    }
}


@pytest.mark.parametrize("data_idx_list, expected_matched_idx", [([1, 5], [3])])
def test_load_models_with_data_idx_list(data_idx_list, expected_matched_idx):
    assert (
        load_models_with_data_idx_list(model_metadata_list, data_idx_list)
        == expected_matched_idx
    )
...
@pytest.mark.parametrize(
    "model_idx_list, expected_matched_idx",
    [
        ([6], [1]),
        ([7], [2]),
        ([8], [3]),
...
def test_load_models_by_model_idx(model_idx_list, expected_matched_idx):
    assert (
        load_models_by_model_idx(model_metadata_list, model_idx_list)
        == expected_matched_idx
    )
...
@pytest.mark.parametrize(
    "data_idx_list, expected_matched_idx",
    [
        ([2], [2, 3]),
        ([4], [1, 3]),
        ([5], [1, 2]),
...
def test_load_models_without_data_idx_list(data_idx_list, expected_matched_idx):
    assert (
        load_models_without_data_idx_list(model_metadata_list, data_idx_list)
        == expected_matched_idx
    )
